...
class SaleOrder(models.Model):
    ...
    def create(self, vals):
        ...
        for order in res:
            if order.website_id:
                # First set the proper multi_wh_line setting based on website configuration
                if order.website_id.use_multi_wh_setting:
                    order.is_multi_wh_line = '1'
                else:
                    order.is_multi_wh_line = '2'
                    if order.website_id.default_warehouse_id:
                        order.warehouse_id = order.website_id.default_warehouse_id

                # Now force update all order lines after is_multi_wh_line is set
                if order.order_line:
                    for line in order.order_line:
                        # Force update warehouse assignments
                        if order.is_multi_wh_line == '2':
                            line.sale_warehouse_id = order.warehouse_id
                            line.sale_warehouse_ids = order.warehouse_id
                        else:
                            line._update_available_warehouse()
                            if order.is_multi_wh_line == '1':
                                line._update_warehouse_line_details()

        return res

    def action_confirm(self):
        if self.is_multi_wh_line == '0' and any(
                not line.sale_warehouse_id and not line.display_type for line in self.order_line):
            for line in self.order_line:
                line._update_available_warehouse()
                line._update_warehouse_line_details()

        if self.website_id:
            old_setting = self.is_multi_wh_line
            if self.website_id.use_multi_wh_setting and self.is_multi_wh_line != '1':
                self.is_multi_wh_line = '1'
                for line in self.order_line:
                    line._update_available_warehouse()
                    line._compute_qty_at_date()
            elif self.website_id.multi_wh_enabled and not self.website_id.use_multi_wh_setting and self.is_multi_wh_line != '2':
                self.is_multi_wh_line = '2'
                if self.website_id.default_warehouse_id:
                    self.warehouse_id = self.website_id.default_warehouse_id
                    # Update all lines to use the warehouse
                    for line in self.order_line:
                        line._update_available_warehouse()
            elif not self.website_id.multi_wh_enabled and self.is_multi_wh_line != '2':
                self.is_multi_wh_line = '2'
                if self.website_id.default_warehouse_id:
                    self.warehouse_id = self.website_id.default_warehouse_id
                    # Update all lines to use the warehouse
            ...
            # If the setting changed, make sure all lines are updated
            if old_setting != self.is_multi_wh_line:
                for line in self.order_line:
                    line._update_available_warehouse()
                    if self.is_multi_wh_line == '1':
                        line._update_warehouse_line_details()
...
class SaleOrderLine(models.Model):
    ...
    def create(self, vals_list):
        ...
        # First ensure warehouses are properly set for all new lines
        for line in lines:
            if line.product_id and (not line.sale_warehouse_id or not line.sale_warehouse_ids):
                if line.order_id.is_multi_wh_line == '2' and line.order_id.warehouse_id:
                    line.sale_warehouse_id = line.order_id.warehouse_id
                    line.sale_warehouse_ids = line.order_id.warehouse_id
                else:
                    line._update_available_warehouse()

        # Then continue with the existing functionality for is_multi_wh_line == '1'
        if any(lines.mapped(lambda l: l.order_id.is_multi_wh_line == '1')):
            for line in lines.filtered(lambda l: l.order_id.is_multi_wh_line == '1'):
                if not line.order_line_warehouse:
                    total_available_qty = 0
                    warehouse_count = 0
                    for w in line.sale_warehouse_ids:
                        warehouse_count += 1
                        available_qty = self.env['stock.quant']._get_available_quantity(line.product_id, w.lot_stock_id)
                        if available_qty >= line.product_uom_qty:
                            ...
                            break
                        elif available_qty > 0 and line.product_uom_qty > total_available_qty:
                            qty = available_qty if line.product_uom_qty - total_available_qty > available_qty else line.product_uom_qty - total_available_qty
                            total_available_qty += available_qty

                            # Last warehouse
                            if total_available_qty < line.product_uom_qty and warehouse_count == len(
                                    line.sale_warehouse_ids):
                                qty = line.product_uom_qty - (total_available_qty - available_qty)
                            ...
                            self.env['sale.order.line.warehouse'].create({
                                ...
                                'company_id': line.company_id.id,
                                'order_line_id': line.id,
                                'product_uom_qty': line.product_uom_qty - total_available_qty,
                                'product_uom': line.product_uom.id
                            })
    ...
    def write(self, values):
        ...
        for o_line in self:
            if not isinstance(o_line.id, models.NewId) and o_line.order_id.is_multi_wh_line == '1' and o_line.sale_warehouse_ids:
                if not o_line.order_line_warehouse:
                    o_line._update_warehouse_line_details()
                elif o_line.order_line_warehouse and 'order_line_warehouse' in values:
                    available_warehouse = []
                    # Delete order line warehouse
                    if len(o_line.order_line_warehouse) < len(o_line.sale_warehouse_ids):
                        for line in o_line.sale_warehouse_ids:
                            if line.id in o_line.order_line_warehouse.mapped("warehouse_id").ids:
                                available_warehouse.append(line.id)

                    # Add new order line warehouse
                    elif len(o_line.order_line_warehouse) > len(o_line.sale_warehouse_ids):
                        for wh_id in o_line.order_line_warehouse.mapped("warehouse_id").ids:
    ...
    def _update_available_warehouse(self):
        ...
        if self.order_id.is_multi_wh_line == '2':
            # Always set both fields when using sale order warehouse
        ...
        # Always ensure both fields are consistent
        if self.sale_warehouse_id and not self.sale_warehouse_ids:
            self.sale_warehouse_ids = self.sale_warehouse_id
        elif self.sale_warehouse_ids and len(self.sale_warehouse_ids) == 1 and not self.sale_warehouse_id:
            self.sale_warehouse_id = self.sale_warehouse_ids
